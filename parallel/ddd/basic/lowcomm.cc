...
#       if DebugLowComm<=6
  sprintf(cBuffer, "%4d: LC_NewSendMsg(%s) dest=%d nSends=%d\n",
          me, mtyp->name, aDest, lcContext.nSends+1);
  DDD_PrintDebug(cBuffer);
#       endif
...
#       if DebugLowComm<=7
  sprintf(cBuffer, "%4d: LC_Connect() nSends=%d nRecvs=%d\n",
          me, lcContext.nSends, lcContext.nRecvs);
  DDD_PrintDebug(cBuffer);
#       endif
